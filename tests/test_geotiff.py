...
def test_basic_geotiff_properties():
    ...
    assert a.coordtrans.srs == '+proj=utm +zone=6 +datum=WGS84 +units=m +no_defs '
    assert type(a) == geotiff.GeoTIFF
    assert a.data[0][10][5] == 55
    assert a.projection == 'PROJCS["WGS 84 / UTM zone 6N",GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0],UNIT["degree",0.0174532925199433],AUTHORITY["EPSG","4326"]],PROJECTION["Transverse_Mercator"],PARAMETER["latitude_of_origin",0],PARAMETER["central_meridian",-147],PARAMETER["scale_factor",0.9996],PARAMETER["false_easting",500000],PARAMETER["false_northing",0],UNIT["metre",1,AUTHORITY["EPSG","9001"]],AUTHORITY["EPSG","32606"]]'
    assert a.proj4 == '+proj=utm +zone=6 +datum=WGS84 +units=m +no_defs '
    assert a.coordtrans(-145, 65) == (594301.0123902442, 7209946.446076041)
    assert a.delx == 30.0
    assert a.dely == -30.0
    ...
    assert a.Lon[0][0] == -146.98614807600831
    assert a.Lon_pxcenter[-1][0] == -146.98582879544688
    assert a.Lat[0][0] == 64.926695025291949
    assert a.Lat_pxcenter[-1][0] == 64.930598198116968
