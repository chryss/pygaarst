...
def _checkstatus(status, line):
    ...
    if newstatus != 0:
        ...
        raise MTLParseError("Cannot parse the following line after status '%s':\n%s" % (STATUSCODE[status], line))

# Function to execute when reading a line in a given state
def _transstat(status, grouppath, dictpath, line):
    if status == 0:
        raise MTLParseError("Status should not be '%s' after reading line:\n%s" % (STATUSCODE[status], line))
    elif status == 1:
        ...
        if oldgroup != grouppath[-1]:
            raise MTLParseError("Reached line '%s' while reading group '%s'." % (line.strip(), grouppath[-1]))
        del grouppath[-1]
        del dictpath[-1]
        try:
...
def parsemeta(metadataloc):
    ...
    if os.path.isdir(metadataloc):
        ...
        if not metalist:
            raise MTLParseError("No files matching metadata file pattern in directory %s." % metadataloc)
        elif len(metalist) > 0:
        ...
        raise MTLParseError("File location %s is unavailable or doesn't contain a suitable metadata file." % metadataloc)
    ...
    with open(metadatafn, 'rU') as fn:
        for line in fn:
            if status == 4:
                ...
                logging.warning("Metadata file %s appears to have extra lines after the end of the metadata. This is probably, but not necessarily, harmless." % metadatafn)
